...
function normalizeInfinitePhases(anyPhases: any): { name: string; seconds: number; kind?: string; note?: string }[] {
    if (!anyPhases) return [];
    ...
    if (Array.isArray(anyPhases)) {
        return anyPhases
            .map((p: any) => {
                const name = String(p?.name || p?.title || p?.key || p?.type || p?.kind || "");
    ...
    if (typeof anyPhases === "object") {
        return Object.entries(anyPhases)
            .map(([k, v]: any) => {
                ...
                const kind = typeof v === "object" ? String(v?.kind || v?.type || k || "") : String(k || "");
                const note =
                    typeof v === "object" ? String(v?.note || v?.description || "").trim() || undefined : undefined;
...
function normalizeBlockKind(raw: any): RoomTimelineBlockKind {
    ...
    if (s.includes("outro") || s.includes("farewell") || s.includes("wrap") || s.includes("closing") || s.includes("end")) {
        return "outro";
    }
...
export function makeDefaultTimelineBlocks(): RoomTimelineBlock[] {
    return [
        { id: uid(), kind: "welcome", title: "Welcome", minutes: 3, note: "Quick intro / rules / vibe" },
        { id: uid(), kind: "intentions", title: "Intentions", minutes: 5, note: "Say what you’ll finish" },
        { id: uid(), kind: "focus", title: "Focus", minutes: 50, note: "Deep work block" },
        { id: uid(), kind: "break", title: "Break", minutes: 10, note: "Recharge / stretch" },
        { id: uid(), kind: "focus", title: "Focus", minutes: 50, note: "Second focus block" },
        { id: uid(), kind: "recap", title: "Recap", minutes: 5, note: "What got done / what’s next" },
        { id: uid(), kind: "celebrate", title: "Celebrate", minutes: 3, note: "Closure + positive finish" },
    ];
}

export function getTimelineTotalMinutes(blocks: RoomTimelineBlock[]) {
    return (blocks || []).reduce((sum, b) => sum + clamp(Number(b.minutes) || 0, 0, 24 * 60), 0);
}
...
export function timelineBlocksFromSchedule(rawSchedule: any): RoomTimelineBlock[] {
    ...
    if (Array.isArray(parsed)) {
        return parsed
            .map((b: any) => {
                const kind = normalizeBlockKind(b?.kind || b?.type || b?.stageType || b?.title || b?.name);
                const title = String(b?.title || b?.name || b?.label || defaultTitleForKind(kind)).trim() || defaultTitleForKind(kind);
    ...
    if (isInfiniteLike) {
        ...
        return phases.map((p) => {
            ...
            return {
                id: uid(),
                kind,
                title: String(p?.name || defaultTitleForKind(kind)).trim() || defaultTitleForKind(kind),
...
const LIBRARY: RoomTimelineBlock[] = [
    { id: "lib_welcome", kind: "welcome", title: "Welcome", minutes: 3, note: "Quick intro / rules / vibe" },
    { id: "lib_intentions", kind: "intentions", title: "Intentions", minutes: 5, note: "Say what you’ll finish" },
    { id: "lib_focus", kind: "focus", title: "Focus", minutes: 50, note: "Deep work block" },
    { id: "lib_break", kind: "break", title: "Break", minutes: 10, note: "Recharge / stretch" },
    { id: "lib_checkin", kind: "checkin", title: "Check-in", minutes: 3, note: "Short accountability checkpoint" },
    { id: "lib_recap", kind: "recap", title: "Recap", minutes: 5, note: "What got done / what’s next" },
    { id: "lib_celebrate", kind: "celebrate", title: "Celebrate", minutes: 3, note: "Closure + positive finish" },
    { id: "lib_outro", kind: "outro", title: "Outro", minutes: 3, note: "Wrap up / closing" },
    { id: "lib_custom", kind: "custom", title: "Custom", minutes: 5, note: "Any special block" },
];
...
export default function RoomTimelineEditor({
    ...
    const studioTotal = useMemo(() => getTimelineTotalMinutes(blocks), [blocks]);
    ...
    return (
        <div className={`fixed inset-0 z-[90] ${overlayBg} p-2 sm:p-4 flex items-center justify-center`}>
            <div className={`w-full max-w-[1200px] max-h-[92vh] rounded-[24px] shadow-2xl border ${subtleBorder} ${panelBg} overflow-hidden flex flex-col`}>
                <div className={`px-4 sm:px-6 py-4 border-b ${subtleBorder} flex items-center justify-between gap-3`}>
                ...
                <div ref={modalScrollRef} className="flex-1 overflow-y-auto p-4 sm:p-6">
                    ...
                    <div className="mt-4 flex items-center justify-between gap-3 flex-wrap">
                        ...
                        <div className="font-inter text-[12px]">
                            Total: <span className="font-semibold">{studioTotal} min</span>
                        </div>
                    </div>
                    ...
                    <div className="mt-4 grid grid-cols-1 lg:grid-cols-2 gap-4">
                                ...
                                {LIBRARY.map((b) => (
                                    ...
                                    >
                                        <div className="flex items-start justify-between gap-2">
                                            <div className="min-w-0">
                                                <div className="font-inter font-semibold text-[12px] truncate">{b.title}</div>
                                                <div className={`mt-1 font-inter text-[11px] ${mutedText} leading-snug`}>
                                                    {b.note}
                                                </div>
                                            </div>
                                            <div className="shrink-0 text-[11px] font-inter opacity-70">{b.minutes}m</div>
                                        </div>
                        ...
                        >
                            ...
                            {!blocks.length ? (
                                <div className={`mt-4 rounded-[16px] border border-dashed ${subtleBorder} p-5 font-inter text-[12px] ${mutedText}`}>
                                ...
                                <div className="mt-4 space-y-3">
                                    {blocks.map((b, idx) => {
                                        ...
                                        const isOverSelf = dragOverId === b.id && draggingId && draggingId !== b.id;
                                        ...
                                        return (
                                            <div
                                                ...
                                                onDragOver={(e) => {
                                                    ...
                                                    const edge: "before" | "after" = e.clientY < mid ? "before" : "after";
                                            ...
                                            >
                                                ...
                                                <div className="flex items-center justify-between gap-2">
                                                    <div className="flex items-center gap-2 min-w-0">
                                                        ...
                                                        <span className={`px-2 py-1 rounded-full border ${subtleBorder} text-[10px] font-inter ${mutedText}`}>
                                                            {b.kind}
                                                        </span>
                                                    </div>
                                                ...
                                                <div className="mt-3 grid grid-cols-1 sm:grid-cols-[160px,minmax(0,1fr),110px] gap-2">
                                                    <select
                                                        ...
                                                        onChange={(e) =>
                                                            updateBlock(b.id, {
                                                                kind: normalizeBlockKind(e.target.value),
                                                                title:
                                                                    String(b.title || "").trim() || defaultTitleForKind(normalizeBlockKind(e.target.value)),
                                                            })
                                                        }
                ...
                <div className={`px-4 sm:px-6 py-4 border-t ${subtleBorder} flex items-center justify-between gap-3`}>
                    <div className={`font-inter text-[12px] ${mutedText}`}>
