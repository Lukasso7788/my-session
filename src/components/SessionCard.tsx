...
function filterActiveRows(rows: any[], cutoffMs: number): any[] {
  return (rows || []).filter((r: any) => {
    ...
    // fallback: if last_seen_at absent, treat recent join as active
    const j = parseTimeMs(r?.joined_at);
    if (j != null) return j >= cutoffMs;
...
async function fetchLiveUsers(sessionId: string): Promise<BookedUser[]> {
  ...
  {
    let usedLastSeen = true;
    const selectWith = "user_id, profiles:profiles(id, full_name, avatar_url), left_at, joined_at, last_seen_at";
    const selectWithout = "user_id, profiles:profiles(id, full_name, avatar_url), left_at, joined_at";
    ...
    if (!res.error && Array.isArray(res.data)) {
      const rows = usedLastSeen ? filterActiveRows(res.data, cutoffMs) : res.data;
...
export default function SessionCard({
  ...
  const hasLiveNow = liveUsersCount > 0;

  const hasStarted = useMemo(() => {
    if (isInfinite) return true;
    if (liveNowDisplay > 0) return true;

    if (!session?.start_time) return false;
    const t = Date.parse(String(session.start_time));
    if (!Number.isFinite(t)) return false;
    return Date.now() >= t;
  }, [isInfinite, liveNowDisplay, session?.start_time]);
  ...
  }, [session?.id, isInfinite, liveNowDisplay, session?.start_time]);
  ...
  // ✅ auto-switch to live tab when someone is actually live
  useEffect(() => {
  ...
  // ✅ Inline shows LIVE users if infinite OR if someone is live; otherwise shows BOOKED
  const inlinePeopleMode: "booked" | "live" = isInfinite ? "live" : hasLiveNow ? "live" : "booked";
          ...
          <div className="flex flex-col xl:flex-row items-start xl:items-center justify-between gap-4 flex-1">
            ...
            <div className="hidden xl:flex items-center gap-6">
              ...
              <div className="text-center">
                <div className="text-[32px] font-bold text-brandBlack">{liveNowDisplay || "—"}</div>
