...
// - Wraps effect.startEffect(...) to always receive a MediaStream (or Promise<MediaStream>)
// - Allows async startEffect (Jitsi effects are often async) — do NOT mark incompatible because of async
//
// ✅ PATCH (SERIALIZED setEffect + SAFE dispose + TIMEOUTS):
// - All track.setEffect calls go through per-track queue (prevents: "setEffect already in progress!")
// - Every effect op has a timeout; if it hangs -> queue is reset (prevents permanent lock)
// - Dispose waits for pending effect ops on that track (with timeout)
//
// ✅ PATCH #1 (CAM TOGGLE AFTER BLUR FIX + NON-BLOCKING):
// - When turning camera OFF and bg is enabled: clear effect BEFORE muting (with short timeout)
// - When turning camera ON: ensure track, unmute ASAP, then re-apply bg in background (non-blocking)
// - Adds reentrancy guard to prevent double-click races
// ============================================================================
...
export class JitsiEngine {
  ...
  private camToggleInFlight = false;
  ...
  private withTimeout<T>(p: Promise<T>, ms: number, label: string): Promise<T> {
    ...
    const timeout = new Promise<T>((_, rej) => {
      to = setTimeout(() => rej(new Error(`[bg] timeout: ${label} after ${ms}ms`)), ms);
    });
  ...
  private async waitEffectIdle(track: any, timeoutMs: number = 6000) {
    const p = this.effectQueueByTrack.get(track);
    if (!p) return;

    try {
      await this.withTimeout(p, timeoutMs, "waitEffectIdle");
    } catch (e) {
      // If chain is stuck (hung setEffect), reset it so UI doesn’t permanently lock
      try {
        this.logEffect(++this.effectOpSeq, "waitEffectIdle TIMEOUT -> reset queue", { err: String(e) });
      } catch { }
      this.effectQueueByTrack.set(track, Promise.resolve());
    }
  }

  private async safeSetEffect(track: any, effect: any, reason: string, timeoutMs: number = 5500) {
    if (!track || typeof track.setEffect !== "function") return;

    await this.runEffectOpOnTrack(track, `setEffect(${reason})`, async () => {
      const attempt = async () => {
        const p = Promise.resolve(track.setEffect(effect));
        await this.withTimeout(p, timeoutMs, `track.setEffect(${reason})`);
      };
  ...
  private async safeDisposeTrack(track: any, reason: string) {
    if (!track) return;
    await this.waitEffectIdle(track, 6000);
    try {
      track.dispose?.();
   ...
   * Effects expect a MediaStream with getTracks().
   *
   * We normalize input to MediaStream OR Promise<MediaStream>, and return whatever effect returns
   * (MediaStream or Promise<MediaStream>) — do NOT force sync.
   */
  private wrapEffectToForceMediaStream(effect: any) {
    if (!effect || typeof effect.startEffect !== "function") return effect;

    const toMediaStream = (streamLike: any): MediaStream | Promise<MediaStream> => {
      const s = streamLike;
      ...
      if (typeof s?.getOriginalStream === "function") {
        ...
        return Promise.resolve(maybe).then((ms) => {
          if (ms && typeof ms.getTracks === "function") return ms as MediaStream;
          throw new Error("[bg] startEffect: getOriginalStream resolved to non-MediaStream");
        });
      }

      throw new Error("[bg] startEffect received non-MediaStream input");
    };

    return {
      ...effect,
      startEffect: (streamLike: any) => {
        const msOrP = toMediaStream(streamLike);
        if (msOrP && typeof (msOrP as any).then === "function") {
          return (msOrP as Promise<MediaStream>).then((ms) => effect.startEffect(ms));
        }
        return effect.startEffect(msOrP as MediaStream);
      },
    };
  }
  ...
  private async createEffectObject(vb: any) {
    ...
    if (typeof effect.startEffect !== "function") {
      this.markEffectsIncompatible(`${pick.kind} effect has no startEffect()`);
      return null;
    }
  ...
  private async clearBgEffectOnTrack(track: any, timeoutMs: number = 5500) {
    if (!track) return;

    const hasSetEffect = typeof track.setEffect === "function";
    if (hasSetEffect) {
      try {
        await this.safeSetEffect(track, this.PASSTHROUGH_EFFECT, "clear", timeoutMs);
      } catch (e) {
  ...
  private async ensureLocalVideoTrack(): Promise<void> {
    ...
    if (oldVideo) {
      try {
        await this.waitEffectIdle(oldVideo, 6000);
      } catch { }
      try {
  ...
  // ========================================================================
  // ✅ PATCH #1: FIX camera toggle after blur/background effect (NON-BLOCKING + TIMEOUTS)
  // ========================================================================
  async toggleVideoMute(): Promise<void> {
    if (!this.localUserId) return;
    if (this.camToggleInFlight) return;

    this.camToggleInFlight = true;
    ...
    const local = this.participants[this.localUserId];
    if (!local || !this.localVideoTrack) {
      this.camToggleInFlight = false;
      return;
    }
    ...
    try {
      ...
      if (isMutedNow) {
        // Turning ON camera — unmute ASAP
        await this.ensureLocalVideoTrack();
        track = this.localVideoTrack!;

        // If effect op stuck — don’t block
        await this.waitEffectIdle(track, 1800);

        await track.unmute?.();
        local.videoMuted = false;
        this.emitParticipants();

        // Re-apply bg AFTER unmute, but don’t block UI
        void this.reapplyBgIfNeeded();
      } else {
        // Turning OFF camera
        // ✅ CRITICAL: clear effect BEFORE muting (but don’t allow long hangs)
        if (this.bgPrefs.mode !== "none") {
          await this.clearBgEffectOnTrack(track, 1600);
        }
