...
// SFU-only (P2P off) Jitsi engine with stable subscriptions, reactions,
// black-video recovery, safe local A/V self-heal, and background effects.
// Background effects strategy: auto => setEffect (if compatible) else replaceTrack pipeline.
...
export class JitsiEngine {
  ...
  private kickAllSubscribedEndpoints(_reason: string) {
    try {
  ...
  private markEffectsIncompatible(reason: string) {
    if (this.effectsCompatibility !== "incompatible") {
      ...
      try {
        console.warn("[bg] setEffect marked INCOMPATIBLE:", reason);
      } catch { }
    }
  }
  ...
  private markEffectsOk() {
    if (this.effectsCompatibility !== "ok") {
      ...
      try {
        console.log("[bg] setEffect marked OK");
      } catch { }
    }
  }
  ...
  // --- local conference track helpers
  ...
  // --- effect support detection + setEffect path
  ...
  private async applyBgEffectToTrack_setEffect(track: any) {
    ...
    try {
      ...
      try {
        if (typeof effect.isSupported === "function" && !effect.isSupported(track)) {
          throw new Error("setEffect isSupported=false");
        }
      } catch { }

      try {
        if (typeof effect.isEnabled === "function" && !effect.isEnabled(track)) {
  ...
  // --- replaceTrack (Canvas/MediaPipe) path
  private async loadCanvasBgFactory(): Promise<((opts: any) => any) | null> {
    ...
    try {
      ...
      if (typeof fn !== "function") {
        console.warn("[bg] backgroundEffect module loaded but no factory export found");
  ...
  private async stopReplaceTrackProcessor(_reason: string) {
    try {
  ...
  private async disableBg_replaceTrack(reason: string, keepPrefs: boolean) {
    ...
    if (processed && base && typeof this.conference.replaceTrack === "function") {
      try {
        ...
        console.warn("[bg] disable replaceTrack(processed->base) failed:", e);
      }
  ...
  private setupConference(roomName: string, userName: string) {
    ...
    // IMPORTANT: set localUserId + create tracks here (was missing earlier)
    conf.on(events.conference.CONFERENCE_JOINED, async () => {
      ...
      try {
        const myId =
          ...
          conf.getMyUserId?.() ||
          conf._myUserId ||
          conf.options?.conference?.myUserId ||
          null;

        this.localUserId = myId || this.localUserId || "local";
        this.ensureLocalParticipant(userName || "Me");
        ...
        console.warn("[conf] CONFERENCE_JOINED handler failed:", e);
      }
    });
